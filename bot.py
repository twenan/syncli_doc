...
def parse_bank_details(raw_text):
    ...
    # Убираем лишние переносы строк и пробелы, заменяем ё на е
    raw_text = re.sub(r'\s+', ' ', raw_text, flags=re.MULTILINE).strip().replace('ё', 'е').replace('Ё', 'Е')
    
    logging.info(f"📌 Полученный текст для парсинга: {raw_text}")
    ...
    patterns = {
        ...
        "ogrnip": r"ОГРНИП:?\s*(\d+)",
        "account_number": r"(?:р\.с|р\.с\.|р/с|расс?\.\s?счет|расс?\s?счет|рас\.счет|рассчетный\s?счет|расчетный\s?счет):?\s*(\d{20})",
        "bank_name": r"(?:банк|наименование банка):?\s*([А-ЯЁа-яё\s]+(?:банк|БАНК|Bank)?)",
        "bik": r"(?:БИК\s?банка|БИК):?\s*(\d{9})",
        "correspondent_account": r"(?:корр?счет|кор\.?\s?счет|корр?\s?счет|корсчет|коррчет|корреспондентский\s?счет|к\.с\.|к\.с|к/с):?\s*(\d{20})",
        "kpp": r"КПП:?\s*(\d{9})",
