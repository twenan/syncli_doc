...
def parse_bank_details(raw_text):
    ...
    patterns = {
        ...
        "inn": r"ИНН:?\s*(\d{10,12})",  # Берем первый найденный ИНН (обычно это ИНН заказчика)
        "ogrnip": r"ОГРНИП:?\s*(\d+)",
        "account_number": r"(?:р\.с|р\.с\.|р/с|расс\. счет|рас\. счет|расс счет|рас счет|рас\.счет|рассчет|расчетный счет|рассчетный счет):?\s*(\d{20})",
        "bank_name": r"(?:банк|наименование банка):?\s*([А-ЯЁа-яё\s]+(?:банк|БАНК|Bank)?)",
        "bik": r"(?:БИК банка|БИК):?\s*(\d{9})",
        "correspondent_account": r"(?:корсчет|коррсчет|кор счет|кор\. счет|корр\. счет|корр счет|корреспондентский счет|к\.с\.|к\.с|к/с):?\s*(\d{20})",
        "kpp": r"КПП:?\s*(\d{9})",
